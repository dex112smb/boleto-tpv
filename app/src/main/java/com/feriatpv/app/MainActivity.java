...
public class MainActivity extends Activity {
  BluetoothAdapter adapter; BluetoothSocket socket; OutputStream out; BluetoothDevice device;
  ...
  View ui(){LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL);r.setPadding(dp(14),dp(12),dp(14),dp(12));r.setBackgroundColor(Color.rgb(7,10,18));
    ...
    for(int i=0;i<6;i++){final int x=i;Button z=btn(ticketText(i),tile(i));z.setTextSize(22);z.setAllCaps(false);GridLayout.LayoutParams p=new GridLayout.LayoutParams();p.width=0;p.height=0;p.columnSpec=GridLayout.spec(i%3,1f);p.rowSpec=GridLayout.spec(i/3,1f);p.setMargins(dp(5),dp(5),dp(5),dp(5));g.addView(z,p);tickets.add(z);z.setOnClickListener(v->select(x,z));}
    LinearLayout s=new LinearLayout(this);s.setOrientation(LinearLayout.VERTICAL);s.setPadding(dp(18),dp(15),dp(18),dp(15));s.setBackground(panel());LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(0,-1,1);sp.setMargins(dp(12),dp(5),0,dp(5));body.addView(s,sp);s.addView(txt("TOTAL",13,true,Color.LTGRAY));total=txt("0,00 €",40,true,Color.WHITE);s.addView(total);s.addView(space(14));s.addView(txt("DINERO RECIBIDO",13,true,Color.LTGRAY));LinearLayout cashrow=new LinearLayout(this);for(int n:new int[]{10,20,50,100}){Button q=btn(n+" €",Color.rgb(24,34,54));cashrow.addView(q,new LinearLayout.LayoutParams(0,dp(50),1));q.setOnClickListener(v->{received=n;update();});}s.addView(cashrow);s.addView(space(14));s.addView(txt("CAMBIO",13,true,Color.LTGRAY));change=txt("0,00 €",34,true,Color.rgb(50,210,125));s.addView(change);s.addView(space(12));Button cash=btn("IMPRIMIR · EFECTIVO",Color.rgb(25,135,72));s.addView(cash,new LinearLayout.LayoutParams(-1,dp(62)));cash.setOnClickListener(v->printSale("Efectivo"));Button card=btn("IMPRIMIR · TARJETA",Color.rgb(20,110,220));LinearLayout.LayoutParams cardp=new LinearLayout.LayoutParams(-1,dp(62));cardp.topMargin=dp(8);s.addView(card,cardp);card.setOnClickListener(v->printSale("Tarjeta"));
    LinearLayout bot=new LinearLayout(this);Button edit=btn("PRECIOS",Color.rgb(32,42,64)),test=btn("PRUEBA IMPRESORA",Color.rgb(32,42,64)),settings=btn("AJUSTES BLUETOOTH",Color.rgb(32,42,64));for(Button x:new Button[]{edit,test,settings}){LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(56),1);p.setMargins(dp(4),dp(8),dp(4),0);bot.addView(x,p);}r.addView(bot);edit.setOnClickListener(v->editPrices());test.setOnClickListener(v->test());settings.setOnClickListener(v->startActivity(new Intent(Settings.ACTION_BLUETOOTH_SETTINGS)));return r;}
  ...
  TextView txt(String s,int z,boolean bold,int c){TextView t=new TextView(this);t.setText(s);t.setTextSize(z);t.setTextColor(c);t.setGravity(Gravity.CENTER_VERTICAL);if(bold)t.setTypeface(null,1);return t;}Button btn(String s,int c){Button b=new Button(this);b.setText(s);b.setTextColor(Color.WHITE);b.setTypeface(null,1);b.setBackground(round(c));return b;}GradientDrawable round(int c){GradientDrawable g=new GradientDrawable();g.setColor(c);g.setCornerRadius(dp(15));g.setStroke(dp(1),Color.rgb(45,63,95));return g;}GradientDrawable panel(){GradientDrawable g=round(Color.rgb(13,20,33));return g;}View space(int h){Space s=new Space(this);s.setLayoutParams(new LinearLayout.LayoutParams(1,dp(h)));return s;}int dp(int x){return Math.round(x*getResources().getDisplayMetrics().density);}void toast(String s){Toast.makeText(this,s,Toast.LENGTH_SHORT).show();}
  void closePrinter(){try{if(out!=null)out.close();}catch(Exception e){}try{if(socket!=null)socket.close();}catch(Exception e){}out=null;socket=null;device=null;}protected void onDestroy(){super.onDestroy();closePrinter();}
}
